...
exports.DEFAULT_GHANA_VAT_LEVY_RULES = void 0;
exports.validateTaxComponent = validateTaxComponent;
exports.evaluateVat = evaluateVat;
exports.evaluateWht = evaluateWht;
exports.buildOrderTaxScenario = buildOrderTaxScenario;
exports.taxPeriodFor = taxPeriodFor;
exports.DEFAULT_GHANA_VAT_LEVY_RULES = [
...
function validateTaxComponent(component) {
    if (!component.transactionId)
        return { ok: false, reason: 'transaction_id_missing' };
    if (!component.componentType)
        return { ok: false, reason: 'component_type_missing' };
    if (!component.payerType || !component.payeeType || !component.supplierType)
        return { ok: false, reason: 'party_classification_missing' };
    if (component.supplierType === 'PAYSTACK')
        return { ok: false, reason: 'paystack_cannot_be_supplier_or_revenue_owner' };
    if (!component.revenueOwner)
        return { ok: false, reason: 'revenue_owner_missing' };
    if (component.revenueOwner === 'ORE' && component.supplierType !== 'ORE')
        return { ok: false, reason: 'ore_revenue_owner_requires_ore_supplier' };
    if (!component.taxCategory)
        return { ok: false, reason: 'tax_category_missing' };
    if (!component.paymentProcessor)
        return { ok: false, reason: 'payment_processor_missing' };
    if (!component.settlementMethod)
        return { ok: false, reason: 'settlement_method_missing' };
    if (!component.contractType)
        return { ok: false, reason: 'contract_type_missing' };
    if (!component.transactionType)
        return { ok: false, reason: 'transaction_type_missing' };
    if (!component.residentStatus)
        return { ok: false, reason: 'resident_status_missing' };
    if (!Number.isInteger(component.grossAmountPesewas) || component.grossAmountPesewas < 0)
        return { ok: false, reason: 'gross_amount_invalid' };
    if (component.taxableAmountPesewas != null && (!Number.isInteger(component.taxableAmountPesewas) || component.taxableAmountPesewas < 0))
        return { ok: false, reason: 'taxable_amount_invalid' };
    return { ok: true };
}
function evaluateVat(params) {
...
function evaluateWht(params) {
    ...
    if (rule.exemption) {
        return { whtStatus: 'EXEMPT', ruleId: rule.ruleId, taxType: rule.taxType, rateBps: rule.rateBps, taxBasePesewas: 0, whtAmountPesewas: 0, thresholdType: rule.thresholdType ?? null, thresholdAmountPesewas: rule.thresholdAmountPesewas ?? null, certificateRequired: !!rule.certificateRequired };
    }
    ...
    let cumulative = params.cumulative;
    if (thresholdType === 'NO_THRESHOLD_RULE') {
        ...
        cumulative = {
            ...
            thresholdTriggerTransactionId: cumulative.priorCumulativeAmountPesewas < thresholdAmount && cumulative.postTransactionCumulativeAmountPesewas >= thresholdAmount
...
function buildOrderTaxScenario(input) {
    const transactionId = `order:${input.orderId}:final`;
    const paymentProcessor = input.paymentProcessor ?? (input.paymentMethod === 'COD' ? 'CASH' : 'PAYSTACK');
    const settlementMethod = input.settlementMethod ?? (input.paymentMethod === 'COD' ? 'COD_CASH' : 'INTERNAL_LEDGER');
    const transactionDate = input.transactionDate ?? new Date();
    const pricingMode = input.pricingMode ?? 'INCLUSIVE';
    const vendorResidentStatus = input.vendorResidentStatus ?? 'UNKNOWN';
    ...
    const dpResidentStatus = input.deliveryPartnerResidentStatus ?? 'UNKNOWN';
...
function taxPeriodFor(date) {
    return `${date.getUTCFullYear()}-${String(date.getUTCMonth() + 1).padStart(2, '0')}`;
}
function activeRules(rules, asOf) {
...
function matches(rule, component) {
...
function amountForBase(component, base) {
    if (base === 'GROSS_AMOUNT')
        return component.grossAmountPesewas;
    if (base === 'TAXABLE_AMOUNT')
        return component.taxableAmountPesewas ?? component.grossAmountPesewas;
    return component.taxableAmountPesewas ?? component.grossAmountPesewas;
}
function normalizeDeliveryPartnerType(value) {
    if (value === 'FLEET_DELIVERY_PARTNER')
        return 'FLEET_DELIVERY_PARTNER';
    if (value === 'DELIVERY_PARTNER')
        return 'DELIVERY_PARTNER';
    if (value === 'INDEPENDENT_DELIVERY_PARTNER')
