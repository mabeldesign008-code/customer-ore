...
export const AdminRole = Object.fromEntries(
  ADMIN_ROLE_ORDER.map((key) => [key.toUpperCase(), key] as const),
) as Record<Uppercase<AdminRoleKey>, AdminRoleKey>;
 ...
 */
export const PERMISSION_MATRIX: Record<string, AccessString> = {
  // ── 1. admin & platform ──────────────────────────────────────────────
  'admin.user.read':                 'R-----R-',
  'admin.user.create':               'D-------',
...
const CODES: readonly Access[] = ['R', 'W', 'D', '-'];

/** Throws on any malformed row. Called at module load — a typo is a boot failure. */
export function validatePermissionMatrix(
  matrix: Record<string, AccessString> = PERMISSION_MATRIX,
): void {
  const width = ADMIN_ROLE_ORDER.length;
  for (const [permission, access] of Object.entries(matrix)) {
    ...
    for (const ch of access) {
      if (!CODES.includes(ch as Access)) {
...
export const DUAL_CONTROLLED_PERMISSIONS: Permission[] = ALL_PERMISSIONS.filter((p) =>
  PERMISSION_MATRIX[p].includes('D'),
) as Permission[];

export function accessFor(permission: string, role: string | null | undefined): Access {
  const row = PERMISSION_MATRIX[permission];
  if (!row) return '-';
  const idx = role ? ADMIN_ROLE_ORDER.indexOf(role as AdminRoleKey) : -1;
  if (idx < 0) return '-';
  return row[idx] as Access;
}
...
export function permissionCensus(): Record<AdminRoleKey, { read: number; write: number; dual: number; total: number }> {
  const out = {} as Record<AdminRoleKey, { read: number; write: number; dual: number; total: number }>;
