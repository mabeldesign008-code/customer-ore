...
export type PerformanceMetricFormula =
  | 'RATE_GTE_TARGET'
  | 'RATE_LTE_TARGET'
  | 'AVERAGE_GTE_TARGET'
  | 'AVERAGE_LTE_TARGET'
...
export interface PerformanceActionRule {
  ...
  whenMetricBelow?: { metricCode: string; scoreBelow: number };
...
export interface PerformanceEngineResult {
  ...
  categoryScores: Array<{ category: PerformanceMetricCategory; score: number | null; weight: number; insufficientData: boolean }>;
  triggeredActions: Array<PerformanceActionRule & { reason: string }>;
}

export const DEFAULT_GRADE_BANDS: PerformanceGradeBand[] = [
...
export const DEFAULT_VENDOR_PERFORMANCE_CONFIG: PerformanceEngineConfig = {
  subject: 'VENDOR',
  version: 1,
...
export const DEFAULT_RIDER_PERFORMANCE_CONFIG: PerformanceEngineConfig = {
  subject: 'RIDER',
  version: 1,
  reviewPeriod: 'MONTHLY',
  minimumTotalSampleSize: 5,
...
export function scorePerformance(input: {
  ...
  const status: PerformanceStatus = insufficientData ? 'INSUFFICIENT_DATA' : band?.status ?? 'MONITOR';
...
function normalizeConfig(config: PerformanceEngineConfig): PerformanceEngineConfig {
  ...
  return { ...config, metrics, gradeBands: config.gradeBands.length ? config.gradeBands : DEFAULT_GRADE_BANDS };
}

function scoreMetric(subject: PerformanceSubject, config: PerformanceMetricConfig, input: PerformanceMetricInput | undefined): PerformanceMetricScore {
  ...
  const defaultExcluded: PerformanceAttribution[] = subject === 'VENDOR'
...
function actualValue(config: PerformanceMetricConfig, input: PerformanceMetricInput | undefined): number | null {
...
function rawScore(config: PerformanceMetricConfig, actual: number | null): number | null {
  if (actual === null || Number.isNaN(actual)) return null;
  let score: number;
  switch (config.formula) {
    case 'RATE_GTE_TARGET':
    case 'AVERAGE_GTE_TARGET': {
      if (config.target <= 0) score = actual > 0 ? 100 : 0;
      else score = (actual / config.target) * 100;
      break;
    }
    case 'RATE_LTE_TARGET':
    case 'AVERAGE_LTE_TARGET': {
      if (actual <= config.target) score = 100;
      else score = config.target <= 0 ? 0 : (config.target / actual) * 100;
      break;
    }
...
function scoreCategories(config: PerformanceEngineConfig, metrics: PerformanceMetricScore[]): PerformanceEngineResult['categoryScores'] {
...
function bandForScore(bands: PerformanceGradeBand[], score: number): PerformanceGradeBand | null {
  return bands.find((band) => score >= band.minScore && score <= (band.maxScore ?? 100)) ?? null;
}

function trendFor(score: number | null, previousScores: number[], rules?: PerformanceTrendRule): PerformanceTrendStatus {
  if (score === null) return 'INSUFFICIENT_DATA';
  if (previousScores.length === 0) return 'NEW';
  ...
  if (delta >= (rules?.improvingDelta ?? 5)) return 'IMPROVING';
  if (delta <= (rules?.decliningDelta ?? -5)) return 'DECLINING';
  return 'STABLE';
}

function actionRulesFor(
  config: PerformanceEngineConfig,
  status: PerformanceStatus,
  metrics: PerformanceMetricScore[],
  overallScore: number | null,
): Array<PerformanceActionRule & { reason: string }> {
  return (config.actionRules ?? []).flatMap((rule) => {
    if (rule.whenStatusIn?.includes(status)) return [{ ...rule, reason: `status:${status}` }];
