...
exports.PgConsumerDedupe = exports.InMemoryConsumerDedupe = void 0;
exports.createConsumerDedupe = createConsumerDedupe;
 ...
 */
class InMemoryConsumerDedupe {
    seen = new Set();
    async release(envelopeId) {
        this.seen.delete(envelopeId);
    }
    async take(envelopeId) {
        if (this.seen.has(envelopeId))
            return false;
...
exports.InMemoryConsumerDedupe = InMemoryConsumerDedupe;
/** Postgres-backed dedupe, safe across replicas and restarts. */
class PgConsumerDedupe {
    ds;
    schema;
    cache = new Set();
    sweepTimer = null;
    ...
    async take(envelopeId) {
        ...
        const result = (await this.ds.query(`INSERT INTO ${this.table()} ("envelopeId") VALUES ($1)
       ON CONFLICT ("envelopeId") DO NOTHING
       RETURNING "envelopeId"`, [envelopeId]));
        const inserted = Array.isArray(result) && result.length > 0;
        if (inserted)
            this.remember(envelopeId);
        return inserted;
    }
    async release(envelopeId) {
    ...
    async sweep() {
        try {
            await this.ensureTable();
            await this.ds.query(`DELETE FROM ${this.table()} WHERE "createdAt" < $1`, [new Date(Date.now() - RETENTION_MS)]);
        }
        catch (err) {
...
exports.PgConsumerDedupe = PgConsumerDedupe;
/**
 ...
 */
function createConsumerDedupe(ds, schema) {
    if (!ds || ds.options.type !== 'postgres')
        return new InMemoryConsumerDedupe();
    const dedupe = new PgConsumerDedupe(ds, schema);
    dedupe.start();
    return dedupe;
}
//# sourceMappingURL=consumer-dedupe.js.map