...
exports.OutboxBus = exports.OutboxStore = void 0;
exports.ensureOutboxTable = ensureOutboxTable;
 ...
 */
function ambientExecutor(ds) {
    try {
        const manager = (0, common_1.getEntityManagerByDataSourceName)((0, common_1.getTransactionalContext)(), 'default');
        return manager ?? ds;
    }
    catch {
        return ds;
...
async function ensureOutboxTable(ds, schema) {
    const has = await ds.query(`SELECT to_regclass('${schema}.${OUTBOX_TABLE}') AS t`);
    if (has[0]?.t) {
...
class OutboxStore {
    ds;
    schema;
    constructor(ds, schema) {
        this.ds = ds;
        this.schema = schema;
    }
    async enqueue(name, payload, envelopeId) {
        await ambientExecutor(this.ds).query(`INSERT INTO ${tableName(this.schema)} ("envelopeId", "eventName", payload, status, attempts)
       VALUES ($1, $2, $3::jsonb, 'pending', 0)`, [envelopeId, name, JSON.stringify(payload)]);
    }
    /**
     ...
     */
    async claim(limit = BATCH_SIZE) {
        const staleBefore = new Date(Date.now() - CLAIM_TIMEOUT_MS);
        const rows = (await this.ds.query(`UPDATE ${tableName(this.schema)} SET status = 'claiming', "claimedAt" = now(), "updatedAt" = now()
       WHERE id IN (
         ...
         payload, status, attempts, error, "createdAt" AS "createdAt", "updatedAt" AS "updatedAt"`, [limit, MAX_ATTEMPTS, staleBefore]));
        // TypeORM UPDATE … RETURNING returns [[rows], affected] for pg driver.
        return Array.isArray(rows) && Array.isArray(rows[0]) ? rows[0] : rows;
    }
    async markSent(id) {
        await this.ds.query(`UPDATE ${tableName(this.schema)} SET status = 'sent', attempts = attempts + 1, "claimedAt" = NULL, "updatedAt" = now() WHERE id = $1`, [id]);
    }
    /**
    ...
    async markFailure(id, error) {
        await this.ds.query(`UPDATE ${tableName(this.schema)} SET attempts = attempts + 1, error = $2, "updatedAt" = now(),
         "claimedAt" = NULL,
         status = CASE WHEN attempts + 1 >= $3 THEN 'failed' ELSE 'pending' END
       WHERE id = $1`, [id, error.slice(0, 500), MAX_ATTEMPTS]);
    }
}
...
class OutboxBus {
    ...
    async publish(name, payload, opts) {
        await this.store.enqueue(name, payload, opts?.envelopeId ?? `${name}:${(0, crypto_1.randomUUID)()}`);
    }
    ...
    async relay() {
        try {
            ...
            for (const row of rows) {
                try {
                    await this.inner.publish(row.eventName, row.payload, { envelopeId: row.envelopeId });
                    ...
                    if (attempts >= MAX_ATTEMPTS) {
                        await this.dlq
                            .publishRaw?.(`${DLQ_SUBJECT_PREFIX}.${row.eventName}`, JSON.stringify({ envelopeId: row.envelopeId, eventName: row.eventName, payload: row.payload, error: msg, ts: new Date().toISOString() }))
                            .catch(() => undefined);
