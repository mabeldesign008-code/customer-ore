...
import { ADMIN_ROLE_ORDER, AdminRoleKey, roleHasPermission } from '@ore/contracts';
import { Role } from '@ore/contracts';
import { internalFetch, serviceUrl } from './http';
import { JwtPayload } from './jwt';
import { IS_INTERNAL_KEY, IS_PUBLIC_KEY, ROLES_KEY } from './guards';

export const PERMISSION_AUDIT_SINK = 'ORE_PERMISSION_AUDIT_SINK';

export const PERMISSIONS_KEY = 'ore_permissions';
 ...
 */
export class RemotePermissionAuditSink implements PermissionAuditSink {
  private readonly logger = new Logger('PermissionAudit');

  record(entry: Parameters<PermissionAuditSink['record']>[0]): void {
    void internalFetch(`${serviceUrl('auth')}/auth/internal/admin-actions`, {
      method: 'POST',
      body: JSON.stringify(entry),
    })
      .then((res) => {
        if (!res.ok) this.logger.warn(`audit post rejected: ${res.status}`);
      })
      .catch((err: Error) => this.logger.warn(`audit post failed: ${err.message}`));
  }
}
...
export class PermissionGuard implements CanActivate {
  ...
  constructor(
    ...
    // Optional: only the auth service owns the audit table. Everywhere else the guard
    // still enforces, it just cannot persist the decision row.
    @Optional()
    @Inject(PERMISSION_AUDIT_SINK)
    private readonly audit: PermissionAuditSink = { record: () => undefined },
  ) {}
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    if (this.reflector.getAllAndOverride<boolean>(IS_PUBLIC_KEY, [handler, target])) return true;
    if (this.reflector.getAllAndOverride<boolean>(IS_INTERNAL_KEY, [handler, target])) return true;

    const req = ctx.switchToHttp().getRequest();
    const user = req.user as JwtPayload | undefined;
    if (!user) return true; // AuthGuard runs first and would already have thrown.

    const required = this.reflector.getAllAndOverride<string[]>(PERMISSIONS_KEY, [handler, target]);
    const roles = this.reflector.getAllAndOverride<Role[]>(ROLES_KEY, [handler, target]) ?? [];
    const adminOnly = roles.length > 0 && roles.every((r) => r === Role.ADMIN);
    ...
    if (!callerRoles.includes(Role.ADMIN)) return true;
    ...
    if (access.status !== 'ACTIVE' && !selfInspectionOnly) {
      ...
      throw new ForbiddenException(`Admin account is ${access.status.toLowerCase()}`);
    }
    ...
    throw new ForbiddenException('This admin endpoint has no permission declared');
  }

  /* ─────────────────────────── helpers ─────────────────────────── */

  private holds(access: AdminAccess, permission: string): boolean {
    ...
    if (permission === 'admin.audit.read_own') return true;
    ...
    if (access.adminRole === 'super_admin') {
      ...
      return roleHasPermission('super_admin', permission);
    }
    if (!access.adminRole || !ADMIN_ROLE_ORDER.includes(access.adminRole as AdminRoleKey)) return false;
    return roleHasPermission(access.adminRole, permission);
  }

  private async adminAccess(user: JwtPayload): Promise<AdminAccess> {
    const cached = this.cache.get(user.sub);
    if (cached && Date.now() - cached.at < CACHE_TTL_MS) return cached.access;
    ...
    try {
      const res = await internalFetch(`${serviceUrl('auth')}/auth/internal/admins/${user.sub}`);
      if (res.ok) {
        const body = (await res.json()) as Partial<AdminAccess> & { found?: boolean };
        if (body?.found) {
          access = {
  ...
  private routeMeta(ctx: ExecutionContext, req: { method?: string; url?: string; ip?: string }) {
  ...
  private async record(
    ...
    decision: 'allow' | 'deny',
    reason: string,
    meta: { service: string; method: string; path: string; ip: string | null },
  ): Promise<void> {
    ...
    if (decision === 'allow' && !permission) return;
    try {
