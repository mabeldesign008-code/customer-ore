...
exports.distanceToPolygonM = distanceToPolygonM;
...
function distanceToPolygonM(point, ring) {
    let min = Infinity;
    for (let i = 0, j = ring.length - 1; i < ring.length; j = i++) {
...
function distToSegmentM(p, a, b) {
    ...
    const r = 6_371_000;
    const toX = (lng, lat) => (lng * Math.PI * r) / 180 * Math.cos((lat * Math.PI) / 180);
    const toY = (lat) => (lat * Math.PI * r) / 180;
