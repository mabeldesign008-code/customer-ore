...
class LiveGeocoder {
    ...
    async google(query) {
        ...
        if (!this.ore.googleMapsApiKey) {
            ...
            try {
                const url = `${this.ore.googleMapsBaseUrl}/geocode/json?address=${encodeURIComponent(query)}` +
        ...
        if (lat !== undefined && lng !== undefined) {
            return { lat, lng, source: contracts_1.GeocoderSource.GOOGLE_MAPS, label };
        }
        ...
        if (this.ore.geocoderOsmFallback) {
            try {
                ...
                const nomBody = (await nomRes.json());
            ...
            if (lat !== undefined && lng !== undefined && Number.isFinite(lat) && Number.isFinite(lng)) {
                const msg = `[maps] geocoding "${query}" served by OpenStreetMap Nominatim because ${googleFailure}. ` +
                ...
                if (this.ore.nodeEnv === 'production')
                    ...
                    console.warn(msg);
                return { lat, lng, source: contracts_1.GeocoderSource.OPENSTREETMAP, label };
            }
        }
        ...
        throw new Error(`Geocoding failed for "${query}" — ${googleFailure}` +
            (this.ore.geocoderOsmFallback ? ' (OSM fallback also returned nothing)' : ' (OSM fallback disabled)'));
    }
    async w3w(words) {
        if (!this.ore.what3wordsApiKey)
            throw new Error('WHAT3WORDS_API_KEY is not set');
        const clean = words.replace(/^\/+/, '');
        const url = `${this.ore.what3wordsBaseUrl}/convert-to-coordinates?words=${encodeURIComponent(clean)}` +
            `&key=${this.ore.what3wordsApiKey}`;
        const res = await fetch(url);
        const body = (await res.json());
        if (!body.coordinates)
            throw new Error(`what3words failed: ${body.error?.message ?? 'unknown'}`);
        return {
            lat: body.coordinates.lat,
            lng: body.coordinates.lng,
            source: contracts_1.GeocoderSource.WHAT3WORDS,
            label: words,
            what3words: words,
            accuracyM: 3,
        };
    }
    async ghanagps(digitalAddress) {
        if (!this.ore.ghanaGpsApiKey)
            throw new Error('GHANA_GPS_API_KEY is not set');
        ...
        return {
            lat,
            lng,
            source: contracts_1.GeocoderSource.GHANA_GPS,
    ...
    async route(a, b) {
        ...
        const url = `${this.ore.googleMapsBaseUrl}/directions/json?origin=${a.lat},${a.lng}` +
...
exports.LiveGeocoder = LiveGeocoder;
