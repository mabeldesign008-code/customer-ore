...
export type LocationInput =
  | { mode: 'auto'; lat: number; lng: number }
  | { mode: 'google'; query: string }
  | { mode: 'w3w'; words: string }
  | { mode: 'ghanagps'; digitalAddress: string };

export interface Geocoder {
  resolve(input: LocationInput): Promise<ResolvedLocation>;
  /** Road distance + duration estimate between two points (for ETA). */
  route(a: { lat: number; lng: number }, b: { lat: number; lng: number }): Promise<{ distanceM: number; durationS: number }>;
}
