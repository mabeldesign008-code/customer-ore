...
import { GeocoderSource, PLACEHOLDER_COORDS } from '@ore/contracts';
import { haversineM } from '@ore/geo';
  ...
  async resolve(input: LocationInput): Promise<ResolvedLocation> {
    switch (input.mode) {
      ...
      case 'google': {
        ...
        if (hit) return { ...hit.loc, source: GeocoderSource.GOOGLE_MAPS, label: hit.key };
      ...
      case 'w3w': {
        ...
        if (hit) return { ...hit[1], source: GeocoderSource.WHAT3WORDS, label: hit[0], what3words: input.words };
      ...
      case 'ghanagps': {
        ...
        if (hit) return { ...hit[1], source: GeocoderSource.GHANA_GPS, label: hit[0], digitalAddress: addr };
...
function findKey(map: Record<string, { lat: number; lng: number }>, query: string): { key: string; loc: { lat: number; lng: number } } | null {
  const q = query.toLowerCase();
  const exact = Object.entries(map).find(([k]) => k.toLowerCase() === q);
  if (exact) return { key: exact[0], loc: exact[1] };
  const partial = Object.entries(map).find(([k]) => k.toLowerCase().includes(q));
  if (partial) return { key: partial[0], loc: partial[1] };
  return null;
}
