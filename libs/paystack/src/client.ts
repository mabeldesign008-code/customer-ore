...
export class PaystackInputError extends Error {}
...
export class PaystackClient {
   ...
   */
  private fail(context: string, res: { status: number; statusText: string }, body: { status?: boolean; message?: string } | undefined): never {
   ...
   */
  private static readonly mockPaid = new Map<string, { amountPesewas: number; currency: string; paidAt: string }>();

  /** Mark a reference paid in mock mode, so `verify` reports success for it and nothing else. */
  static markMockPaid(reference: string, amountPesewas: number, currency = 'GHS', paidAt = new Date().toISOString()): void {
  ...
  async verify(reference: string): Promise<VerifyResult> {
    ...
    if (!res.ok || !body.status || !body.data) {
      this.fail(`Paystack verify failed for ${reference}`, res, { status: body?.status, message: (body as any)?.message ?? JSON.stringify(body).slice(0, 200) });
    }
    return {
      reference,
  ...
  async refund(transactionReference: string, amountPesewas?: number, reason?: string): Promise<RefundResult> {
    if (this.isMock) {
    ...
    if (!res.ok || !body.status || !body.data) {
      this.fail('Paystack refund failed', res, { status: body?.status, message: (body as any)?.message ?? JSON.stringify(body).slice(0, 200) });
    }
    return {
      reference: body.data.reference,
  ...
  async createRecipient(params: {
    ...
    bankCode: string; // e.g. "MTN", "VOD", "ATL", or GHIPSS Bank Code
    currency?: string;
  }): Promise<{ recipientCode: string; mode: 'live' | 'mock' }> {
    if (this.isMock) {
      return {
    ...
    const body = (await res.json()) as { status: boolean; message?: string; data?: { recipient_code: string } };
    if (!res.ok || !body.status || !body.data) {
      this.fail('Paystack transfer recipient failed', res, body);
    }
    return { recipientCode: body.data.recipient_code, mode: 'live' };
  }
  async transferToRecipient(params: { reference: string; amountPesewas: number; recipientCode: string; currency?: string; reason?: string; metadata?: Record<string, unknown> }): Promise<TransferResult> {
    if (this.isMock) return { reference: params.reference, status: 'success' };
    ...
    const body = await res.json() as { status: boolean; message?: string; data?: { reference: string; status: string } };
    if (!res.ok || !body.status || !body.data) this.fail('Paystack transfer failed', res, body);
  ...
  async transfer(params: {
    ...
    amountPesewas: number;
    currency: string;
    destination: string; // e.g. "2332… MOMO MTN" — the rider's declared payout account
    reason?: string;
    metadata?: Record<string, unknown>;
  ...
  async getTransferStatus(reference: string): Promise<{ reference: string; status: 'success' | 'processing' | 'failed' }> {
    ...
    const status: 'success' | 'processing' | 'failed' = 
      s === 'success' ? 'success' : s === 'failed' || s === 'reversed' ? 'failed' : 'processing';
    return { reference, status };
  }

  async listGhanaPayoutProviders(type: 'mobile_money' | 'ghipss'): Promise<{ name: string; code: string; type: string }[]> {
    if (this.isMock) {
